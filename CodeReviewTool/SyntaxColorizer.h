...
protected:
	void addKey(CString Keyword, CHARFORMAT cf, int grp);
	void createTables();
	void deleteTables();
	void createDefaultKeywordList();
	void createDefaultCharFormat();
	void colorize(CString lpszBuf, CRichEditCtrl *pCtrl, long iOffset = 0);
...
public:
	...
	void AddKeyword(CString Keyword, CHARFORMAT cf, int grp = 0); 
	void AddKeyword(CString Keyword, COLORREF cr, int grp = 0);
