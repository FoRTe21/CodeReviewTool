...
/*	m_cfDefault.dwMask = CFM_CHARSET | CFM_FACE | CFM_SIZE | CFM_OFFSET | CFM_COLOR;
	...
	wcscpy_s(m_cfDefault.szFaceName, _countof(m_cfDefault.szFaceName) + 1, L"Courier New");
	...
	*/
	...
	m_cfDefault.dwMask = CFM_BOLD | CFM_ITALIC | CFM_STRIKEOUT | CFM_UNDERLINE | CFM_FACE | CFM_SIZE | CFM_CHARSET | CFM_PROTECTED | CFM_COLOR;
	m_cfDefault.yHeight = 200;
	m_cfDefault.crTextColor = RGB(0, 0, 0);
	m_cfDefault.bCharSet = HANGEUL_CHARSET;
	wcscpy_s(m_cfDefault.szFaceName, _countof(m_cfDefault.szFaceName) + 1, L"Courier New");
...
{
	CString sKeywords = L"__asm,else,main,struct,__assume,enum,"
	...
	CString sDirectives = L"#define,#elif,#else,#endif,#error,#ifdef,"
		"#ifndef,#import,#include,#line,#pragma,#undef";
	CString sPragmas = L"alloc_text,comment,init_seg1,optimize,auto_inline,"
...
void CSyntaxColorizer::AddKeyword(CString Keyword, COLORREF cr, int grp)
{
	CString token;
	int index = 0;
	...
	token = keyword.Tokenize(L",", index);
	while(token != L"")
	{
		if(token.CompareNoCase(L"rem") == 0)
...
void CSyntaxColorizer::AddKeyword(CString Keyword, CHARFORMAT cf, int grp)
{
	LPTSTR token;
	LPTSTR context;
	//make a copy of Keyword so that strtok will operate correctly
	LPTSTR keyword = new TCHAR[wcslen(Keyword) + 1];
	wcscpy_s(keyword, wcslen(Keyword) + 1, Keyword);

	token = wcstok_s(keyword,L",", &context);
	while(token != NULL)
	{
		if(_wcsicmp(token,L"rem") == 0)
			*(m_pTableTwo + L'\n') = SLEND; //set single quote as comment start
		addKey(token,cf,grp);
...
void CSyntaxColorizer::addKey(CString Keyword, CHARFORMAT cf, int grp) //add in ascending order
{
	...
	pskNewKey->keylen = Keyword.GetLength();
	pskNewKey->cf = cf;
	pskNewKey->group = grp;
	...
	*(m_pTableZero + pskNewKey->keyword[0]) = KEYWORD;
	...
	else
	{
		...
		else
		{
			...
			while(curr != NULL && wcscmp(curr->keyword,Keyword) < 0)
			{
				prev = curr;
				curr = curr->pNext;
			}
			if(curr != NULL && wcscmp(curr->keyword,Keyword) == 0)
			{
				//the keyword exists, so replace the existing with the new
...
void CSyntaxColorizer::ClearKeywordList()
{
	...
	while(m_pskKeyword != NULL)
	{
		*(m_pTableZero + m_pskKeyword->keyword[0]) = SKIP;
		if(_wcsicmp(m_pskKeyword->keyword,L"rem") == 0)
			*(m_pTableTwo + L'\n') = SKIP;
		pTemp = m_pskKeyword->pNext;
		m_pskKeyword->keyword.Empty();
		delete m_pskKeyword;
		m_pskKeyword = pTemp;
	}
}
...
void CSyntaxColorizer::Colorize(CHARRANGE cr, CRichEditCtrl *pCtrl)
{
	...
	CString lpszBuf;
	lpszBuf = pCtrl->GetSelText();
	pCtrl->SetSelectionCharFormat(m_cfDefault);

	colorize((LPTSTR)(LPCTSTR)lpszBuf,pCtrl,cr.cpMin);
...
void CSyntaxColorizer::Colorize(long nStartChar, long nEndChar, CRichEditCtrl *pCtrl)
{
	...
	colorize((LPTSTR)(LPCTSTR)lpszBuf,pCtrl,nStartChar);

}

void CSyntaxColorizer::colorize(CString lpszBuf, CRichEditCtrl *pCtrl, long iOffset /*=0*/)
{
	//setup some vars
	CHARFORMAT cf;
	CString lpszTemp;
	...
	while(lpszBuf[x])
	{
		...
		case MLEND:
			...
			break;
		case SLEND:
			if(lpszBuf.GetAt(x-2) != L'\\') // line continuation character
			{
				pTable = m_pTableZero;
				pCtrl->SetSel(iStart,iOffset + x+1);
				pCtrl->SetSelectionCharFormat(m_cfComment);
		...
		case DQEND:
			...
			break;
		case SQEND:
			if(lpszBuf.GetAt(x-1) == L'\\' && lpszBuf.GetAt(x+1) == L'\'')
				break;
			pTable = m_pTableZero;
			pCtrl->SetSel(iStart,iOffset + x+1);
			pCtrl->SetSelectionCharFormat(m_cfString);
			break;
		case KEYWORD:
			lpszTemp = lpszBuf.Mid(x);
			while(pskTemp != NULL)
			{
				if(pskTemp->keyword.GetAt(0) == lpszTemp.GetAt(0))
				{
					int x1=0,y1=0;iStart = iOffset + x;
					while(pskTemp->keyword.GetAt(x1))
					{
						y1 += lpszTemp.GetAt(x1) ^ pskTemp->keyword.GetAt(x1);
						x1++;
					}
					if(y1 == 0 && (*(m_pAllowable + (lpszBuf.GetAt(x-1))) && 
							*(m_pAllowable + (lpszBuf.GetAt(x+pskTemp->keylen)))))
					{
						if(pskTemp->keyword.CompareNoCase(L"rem") == 0)
						{
							pTable = m_pTableThree;
						}
						else 
